...
# Size (sqft)	Number of Bedrooms	Number of floors	Age of Home inside each array of X_train.
# Price (1000s dollars) inside the y_train array.
X_train = np.array([[2104, 5, 1, 45], [1416, 3, 2, 40], [852, 2, 1, 35]])
y_train = np.array([460, 232, 178])

# Matrix X containing our examples
'''
Each row of the matrix represents one example. When you have  𝑚training examples ( 𝑚 is three in our example), and there are 𝑛 features (four in our example),
𝐗 is a matrix with dimensions (𝑚, 𝑛) (m rows, n columns).
'''
...
m,_ = X_train.shape
for i in range(m):
    print(f"prediction: {np.dot(X_train[i], w_final) + b_final:0.2f}, target value: {y_train[i]}") #The prediction is the price value of the house and the target is the Price (1000s dollars).
